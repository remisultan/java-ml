...
public class PrincipalComponentAnalysisTest {
  ...
  private static Stream<Arguments> params_must_apply_PCA_to_dataframe() {
    return Stream.of(
        Arguments.of(new PrincipalComponentAnalysis(1).setResponseVariable("strColumn"), 2, 0.707695682508321),
        Arguments.of(new PrincipalComponentAnalysis(2).setResponseVariable("strColumn"), 3, 0.707696924134227),
        Arguments.of(new PrincipalComponentAnalysis(3).setResponseVariable("strColumn"), 4, 0.7080191994986393),
  ...
  public void must_apply_PCA_to_dataframe(
    ...
    assertThat(cosineSim(reconstruct.mapWithout("strColumn").toMatrix(),
        df.mapWithout("strColumn").toMatrix())).isEqualTo(reconstructSimilarity);
  }
}
